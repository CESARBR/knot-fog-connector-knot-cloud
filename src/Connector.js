...
class Connector {
  ...
  async start() {
    ...
    clients.forEach(value => value.client.on('command', (cmd) => {
      let args;
      switch (cmd.payload.name) {
        case 'getData':
          ({ args } = cmd.payload);
          args.forEach(sensorId => this.onDataRequestedCb(value.id, sensorId));
          break;
        case 'setData':
          ({ args } = cmd.payload);
          args.forEach(({ sensorId, data }) => this.onDataRequestedCb(value.id, sensorId, data));
          break;
        default:
