...
class Connector {
  ...
  async resetTokenAndConnect(settings, device) {
    const client = await this.createConnection(settings.uuid, settings.token);
  ...
  async start() {
    ...
    const clients = await Promise.all(devices.map(device => (
      this.resetTokenAndConnect(this.settings, device)
    )));
