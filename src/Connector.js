...
class Connector {
  ...
  async start() {
    ...
    for (const device of devices) {
      ...
      this.clientThings[device.id] = await this.createConnection(device.uuid, await promisify(tmpClient, 'created', tmpClient.createSessionToken.bind(tmpClient), device.uuid));
      tmpClient.close();
    }
  }
